	...
	public function index(CommentViewModel $viewModel)
	{
		...
		$result = $this->service->handle(
			...
			useMapQueryResult(function ($value)  use ($excepts, $properties) {
				return $value ? CommentDto::new($value)->addProperties($properties)->mergeHidden(array_merge($excepts, $value->getHidden() ?? [])) : $value;
			})
		);
	...
	public function show(CommentViewModel $viewModel, $id)
	{
		...
		$result = $this->service->handle(
			...
			function ($value)  use ($excepts, $properties) {
				return null !== $value ? CommentDto::new($value)->addProperties($properties)->mergeHidden(array_merge($excepts, $value->getHidden() ?? [])) : $value;
			}
		);
		return $this->response->create($result);
	}
