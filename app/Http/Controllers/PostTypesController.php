	...
	public function index(PostTypeViewModel $viewModel)
	{
		...
		$result = $this->service->handle(
			...
			useMapQueryResult(function ($value)  use ($excepts, $columns) {
				return $value ? PostTypeDto::new($value)->addProperties($columns)->mergeHidden($excepts) : $value;
			})
		);
	...
	public function show(PostTypeViewModel $viewModel, $id)
	{
		...
		$result = $this->service->handle(
			...
			function ($value)  use ($excepts, $columns) {
				return null !== $value ? PostTypeDto::new($value)->addProperties($columns)->mergeHidden($excepts) : $value;
			}
		);
		return $this->response->create($result);
	}
